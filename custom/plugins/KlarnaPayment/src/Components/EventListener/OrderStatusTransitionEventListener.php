    ...
    private function isFraudStatusAccepted(OrderEntity $orderEntity): bool
    {
        foreach ($this->stateDataHelper->getValidTransactions($orderEntity) as $transaction) {
            if (is_array($transaction->getCustomFields())
                && isset($transaction->getCustomFields()['klarna_fraud_status'])
                && strtolower($transaction->getCustomFields()['klarna_fraud_status']) === strtolower(AbstractKlarnaPaymentHandler::FRAUD_STATUS_ACCEPTED)) {
                return true;
            }
        }
