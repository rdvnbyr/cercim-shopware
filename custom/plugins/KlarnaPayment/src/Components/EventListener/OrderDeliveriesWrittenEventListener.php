...
class OrderDeliveriesWrittenEventListener implements EventSubscriberInterface
{
    ...
    private function updateShippingInfo(OrderDeliveryEntity $orderDelivery, Context $context): void
    {
        ...
        /** @var array<string,mixed> $klarnaValues */
        $klarnaValues  = $transaction->getCustomFields();
        $klarnaOrderId = $klarnaValues['klarna_order_id'];

        $captureId = $this->getCaptureId($klarnaOrderId, $order->getSalesChannelId(), $context);
    ...
    private function getCaptureId(string $klarnaOrderId, string $salesChannelId, Context $context): ?string
    {
        $captureId = null;
